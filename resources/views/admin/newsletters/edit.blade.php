            ...
            <div class="card-body">
              <form action="{{route('admin.newsletters.update', $newsletter)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="title" value="{{ old('title') ? old('title') : $newsletter->title }}" class="form-control" id="headline" placeholder="Enter the title of the video" autocomplete="off">
                  @error('title')
                      <p>{{$message}}</p>
                  @enderror
                </div>
...
<script>
  ...
  .create(document.querySelector('#editor')).then(editor => {
       editor.setData('{!! $newsletter->description !!}');
   });
